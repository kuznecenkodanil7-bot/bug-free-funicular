...
public final class SmoothAssets {
    ...
    private static final int ICON_SOURCE_SIZE = 256;
    private static final Identifier ATLAS_ID = Identifier.of(RaidMineStaffMod.MOD_ID, "runtime/smooth_ui");
    private static final Identifier LOGO_ID = Identifier.of(RaidMineStaffMod.MOD_ID, "runtime/rm_logo_linear");
    ...
    private static volatile boolean logoInitialized;
    private static volatile boolean logoFailed;
    private static int logoWidth = 1;
    private static int logoHeight = 1;
    ...
    public static boolean ensureLogoInitialized() {
        if (logoInitialized) return true;
        if (logoFailed) return false;
        synchronized (SmoothAssets.class) {
            if (logoInitialized) return true;
            try (InputStream stream = SmoothAssets.class.getResourceAsStream(
                    "/assets/raidmine_staff/textures/gui/rm_logo.png")) {
                if (stream == null) {
                    throw new IllegalStateException("RM Tools logo resource is missing");
                }
                NativeImage image = NativeImage.read(stream);
                logoWidth = Math.max(1, image.getWidth());
                logoHeight = Math.max(1, image.getHeight());
                MinecraftClient.getInstance().getTextureManager().registerTexture(
                        LOGO_ID,
                        new LinearTexture(() -> "RM Tools linear logo", image)
                );
                logoInitialized = true;
                return true;
            } catch (Throwable throwable) {
                logoFailed = true;
                RaidMineStaffMod.LOGGER.error("Could not create smooth RM Tools logo texture", throwable);
                return false;
            }
        }
    }
    ...
    public static boolean drawLogoTint(DrawContext context, int x, int y, int boxWidth, int boxHeight, int tint) {
        if (!ensureLogoInitialized() || boxWidth <= 0 || boxHeight <= 0) return false;
        float scale = Math.min(boxWidth / (float) logoWidth, boxHeight / (float) logoHeight);
        int width = Math.max(1, Math.round(logoWidth * scale));
        int height = Math.max(1, Math.round(logoHeight * scale));
    ...
    private static Font createUiFont(boolean bold) {
        ...
        String[] preferred = configured == null || configured.equalsIgnoreCase("AUTO")
                ? automatic
                : new String[]{configured, "Segoe UI Variable", "Inter", "Segoe UI", "Noto Sans", "Roboto", "Arial", Font.SANS_SERIF};
        ...
        attributes.put(TextAttribute.TRACKING, -0.004F);
    ...
    private static void drawIcon(Graphics2D g, UiIcon icon) {
        ...
        switch (icon) {
            ...
            case EYE -> {
                Path2D p = new Path2D.Float(); p.moveTo(8,48); p.curveTo(25,20,71,20,88,48); p.curveTo(71,76,25,76,8,48); p.closePath(); g.draw(p);
                g.draw(new Ellipse2D.Float(34,34,28,28)); g.fill(new Ellipse2D.Float(43,43,10,10));
            }
            ...
            case FOLDER -> {
                Path2D p = new Path2D.Float(); p.moveTo(10,28); p.lineTo(37,28); p.lineTo(45,19); p.lineTo(84,19); p.lineTo(88,78); p.lineTo(10,78); p.closePath(); g.draw(p);
                g.draw(new Line2D.Float(11,38,87,38));
            }
            case RELOAD -> {
                g.drawArc(15,15,66,66,35,285); g.draw(new Line2D.Float(72,14,82,34)); g.draw(new Line2D.Float(82,34,61,31));
            }
            case FONT -> {
                g.draw(new Line2D.Float(20,79,43,17)); g.draw(new Line2D.Float(43,17,66,79)); g.draw(new Line2D.Float(28,57,58,57));
                g.draw(new Line2D.Float(66,36,86,36)); g.draw(new Line2D.Float(76,36,76,80));
            }
            case TIMER -> {
                g.draw(new Ellipse2D.Float(15,20,66,66)); g.draw(new Line2D.Float(48,5,48,20)); g.draw(new Line2D.Float(36,5,60,5));
                g.draw(new Line2D.Float(48,34,48,53)); g.draw(new Line2D.Float(48,53,63,61));
            }
        }
    }
