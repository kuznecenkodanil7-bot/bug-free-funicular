...
/** Resolves only the author of a rendered chat line and tolerates SmartChat offsets/scaling. */
public final class NicknameHitConsumer implements DrawnTextConsumer {
    private static final int LINE_HEIGHT = 9;
    private static final float HORIZONTAL_TOLERANCE = 12F;
    private static final float VERTICAL_TOLERANCE = 28F;
    ...
    public void text(Alignment alignment, int x, int y, Transformation transformation, OrderedText text) {
        ...
        if (scissor != null && !expandedContains(scissor, clickX, clickY, 30)) return;
        ...
        // SmartChat can apply an additional visual vertical offset after the
        // vanilla chat layout. Keep the clickable author, but move the hover
        // decoration to the line under the actual cursor when such an offset
        // is detected.
        float shownTop = top;
        float shownBottom = bottom;
        if (clickY < top || clickY > bottom) {
            float center = (top + bottom) * 0.5F;
            float shift = Math.max(-VERTICAL_TOLERANCE, Math.min(VERTICAL_TOLERANCE, clickY - center));
            shownTop += shift;
            shownBottom += shift;
        }

        hit = new Hit(author.name(), line.substring(author.start(), author.end()),
                Math.round(left), Math.round(shownTop), Math.round(right), Math.round(shownBottom));
    }
    ...
    public Optional<String> nickname() { return hit().map(Hit::nickname); }
    public Optional<Hit> hit() { return Optional.ofNullable(hit); }
